...
PostgreSQL 마이그레이션은 **실제 PostgreSQL 위에서 돌려 검증**한다(`PostgresSchemaTest`).
H2 테스트만으로는 벤더 차이가 절대 드러나지 않기 때문이다.
Testcontainers 가 더 일반적이지만 테스트에 Docker 를 요구하게 되므로,
바이너리를 직접 띄우는 임베디드 PostgreSQL 을 썼다 — 클론해서 `./gradlew test` 만 치면 돈다.
...
50건. 큐 상태 전이(백오프 증가·시도 상한·lease 회수), 정규화 규칙(SKU 최저가·중복 옵션 접기),
수집 플로우(멱등 등록·동시성 상한·워커 소유권), 마켓 규칙 선검사와 판매가 계산,
등록 플로우(마켓별 추적·재전송 멱등·상품ID 없는 성공 응답·전송 타임아웃 회수),
트랜잭션 경계 회귀 테스트,
그리고 실제 PostgreSQL 위에서의 스키마·파이프라인 검증을 다룬다.
